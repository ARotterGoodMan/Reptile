...
class Download(object):
                ...
                async for line in f:
                    ...
                    if line.startswith("http"):
                        ...
                        if line.startswith("/"):
                            ...
                            http_top = http_top.split(f'/{http_top_del}')[0]
                            line = http_top + '/' + line
                            line = line.strip()
                    indexes.append(line)
                    ts_url = line
    ...
    async def merge_ts(self):
        async with aiofiles.open("video_ts\\merge.bat", mode="a", encoding="utf-8") as merge:
            ...
            await merge.write(f" {self.video_name}.mp4")
        self.update_merge_bat()
        self.run_bat()

    def update_merge_bat(self):
        merge_read = open("video_ts\\merge.bat", mode="r", encoding="utf-8")
        line2 = merge_read.read().replace(f"+ {self.video_name}.mp4", f" {self.video_name}.mp4")
        merge_read.close()
        merge_update = open("video_ts\\merge.bat", mode="w", encoding="utf-8")
        merge_update.write(line2)
        merge_update.write('\nmkdir movie\n')
        merge_update.write(f'move {self.video_name}.mp4 movie\n')
        merge_update.close()
        run_merge = open("run.bat", mode="w", encoding="utf-8")
        run_merge.write("chcp 65001\n")
